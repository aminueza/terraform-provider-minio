...
define building_provider
	echo "Building terraform-provider-s3minio_${VERSION}_linux_amd64..."
	env GOOS=linux GOARCH=amd64 $(GO) build -o terraform-provider-s3minio_v${VERSION}_linux_amd64 ./main.go
	echo "Building terraform-provider-s3minio_${VERSION}_darwin_amd64..."
	env GOOS=darwin GOARCH=amd64 $(GO) build -o terraform-provider-s3minio_v${VERSION}_darwin_amd64 ./main.go
	# warning for user
